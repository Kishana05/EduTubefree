...
// Import the EduTubeSync functions for automatic synchronization
import '../../sync-courses.js';
import { 
  Plus, 
  Edit, Trash,
  Eye, X, Youtube,
...
const AdminDashboard: React.FC = () => {
  ...
  const handleUpdateCourse = async (courseData: Course) => {
    ...
    .then(data => {
      ...
      setError(`Course "${courseToAdd.title}" successfully saved to MongoDB Atlas!`);
      ...
      return fetch(`${API_BASE_URL}/api/courses`, {
        ...
        headers: {
          ...
          'x-auth-token': token, // Add the token in the format expected by the middleware
    ...
    .then(response => {
      if (!response.ok) {
        throw new Error(`Failed to refresh courses: ${response.status}`);
      }
      return response.json();
    })
    .then(refreshedCourses => {
